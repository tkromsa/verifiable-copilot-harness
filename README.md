...
- v0.7.1 embeds the starter contract as the `_STARTER` sheet (ADR-019): one attached workbook per session. `copilotstart.txt` stays in the repo as the source of truth and the lint asserts sheet and file match.
...
copilot/
  copilotstart.txt            <- source of truth for the embedded _STARTER sheet
...
1. Attach `core/VCH_HarnessCore.xlsx` (the starter is embedded as the `_STARTER` sheet).
2. Type `load vch`.
3. Ask normally or use an explicit skill trigger.

### Start a project

1. Attach `core/VCH_HarnessCore.xlsx`.
...
- `_STARTER` sheet reproduces `copilot/copilotstart.txt` (ADR-019),
